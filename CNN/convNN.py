...
def driveCNN( dataset_properties, cut_off_properties,model_properties,training_properties, logfolder,f=None ):

    pass
    csv_path, dt_dset, rcpower_dset, discret = dataset_properties
    test_cut_off,val_cut_off = cut_off_properties
    filters,kernel_size,pool_size =  model_properties
...
# set model
    model = setCNNModel( n_steps, n_features, filters, kernel_size,pool_size,   f)
...
def main():
    ...
    model, history, scaler  = driveCNN(dataset_properties, cut_off_properties,model_properties,training_properties, logfolder,  f)


    model_saving(FOLDER_PATH_SAVED_CNN_MODEL, model, scaler, f)
    return 0
