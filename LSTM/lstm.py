...
# we following by (*) https://towardsdatascience.com/3-steps-to-forecast-time-series-lstm-with-tensorflow-keras-ba88c6f05237
#
#
# import packages
import math
...
from predict.api import create_ts_files, TimeSeriesLoader,dsets_logging
from predict.cfg import RCPOWER_DSET, DT_DSET,CSV_PATH, LOG_FILE_NAME
...
if __name__ == "__main__":
    ...
    file_for_logging=LOG_FILE_NAME + Path(__file__).stem + ".log"
    ...
    dsets_logging(dt_dset, rcpower_dset, df_train, df_val, df_test, f)
    ...
    # (*) found that the easiest way to do time series with tensorflow is by creating pandas files with the lagged time
    # steps (eg. x{t-1}, x{t-2}...) and the value to predict y = x{t+n}.
    # We tried doing it using TFRecords, but that API is not very intuitive and lacks working examples for time series.
    # The resulting file using these parameters is over 17GB. If history_length is increased, or  step_size is
    # decreased, it could get much bigger.
    ...
    target_step = 0 # The time step in the future to predict.
    #                                                  Eg. If target_step = 0, then predict the next timestep
    #                                                                           after the end of the history period.
    #                                                       If target_step = 10 then predict 10 timesteps the next
    #                                                       timestep (11 minutes after the end of history).
    # The csv creation returns the number of rows and number of features. We need these values below.
